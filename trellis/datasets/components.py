...
class TextConditionedMixin:
    def __init__(self, roots, **kwargs):
        ...
        for instance in self.instances:
            sha256 = instance[1]
            self.captions[sha256] = self.metadata.loc[sha256]['captions'] # [self.metadata.loc[sha256]['captions'][2:-2]] # json.loads(self.metadata.loc[sha256]['captions'])
    
    def filter_metadata(self, metadata):
        metadata, stats = super().filter_metadata(metadata)
        metadata = metadata[metadata['captions'].notna()]
        stats['With captions'] = 1 #len(metadata)
        return metadata, stats
    
    def get_instance(self, root, instance):
        pack = super().get_instance(root, instance)
        text = json2prompt(self.captions[instance]) # np.random.choice(self.captions[instance])
        pack['cond'] = text
        return pack
